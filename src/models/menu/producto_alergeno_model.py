...
from sqlalchemy import String, Boolean, Enum, ForeignKey, Index, UniqueConstraint
...
class ProductoAlergenoModel(BaseModel, AuditMixin):
    ...
    id_alergeno: Mapped[str] = mapped_column(
        ForeignKey("alergenos.id", ondelete="RESTRICT"), nullable=False, index=True
    )
    ...
    producto: Mapped["ProductoModel"] = relationship(
        "ProductoModel", back_populates="productos_alergenos"
    )

    alergeno: Mapped["AlergenoModel"] = relationship(
        "AlergenoModel", back_populates="productos_alergenos"
    )
    ...
    def update_from_dict(self, data: dict) -> None:
        ...
        data : dict
            ...
            Los campos 'id', 'id_producto' y 'id_alergeno' serán ignorados.
        """
        protected_fks = {"id_producto", "id_alergeno"}
        for key, value in data.items():
            if key not in protected_fks and hasattr(self, key):
