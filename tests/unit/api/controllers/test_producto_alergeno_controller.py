...
from src.api.controllers.producto_alergeno_controller import (
    router,
...
def sample_producto_id():
    ...
    return (
        "12345678-1234-5678-1234-567812345678"  # UUID v4 string (backward compatible)
    )
...
def sample_alergeno_id():
    ...
    return (
        "87654321-4321-8765-4321-876543218765"  # UUID v4 string (backward compatible)
    )
...
def test_create_producto_alergeno_success(
    ...
    mock_producto_alergeno_service,
    sample_producto_id,
    sample_alergeno_id,
    ...
    with patch(
        ...
        response = test_client.post(
            "/api/v1/productos-alergenos", json=producto_alergeno_data
        )

        # Assert
        assert response.status_code == 201
        assert (
            response.json()["id_producto"]
            == sample_producto_alergeno_data["id_producto"]
        )
        assert (
            response.json()["id_alergeno"]
            == sample_producto_alergeno_data["id_alergeno"]
        )
        assert (
            response.json()["nivel_presencia"]
            == sample_producto_alergeno_data["nivel_presencia"]
        )
...
def test_list_producto_alergenos_success(
    ...
    with patch(
        ...
        mock_producto_alergeno_service.get_producto_alergenos.assert_awaited_once_with(
            0, 10
        )
